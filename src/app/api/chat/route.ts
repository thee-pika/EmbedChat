...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!message || !ownerId) {
            return NextResponse.json({ message: "ownerId & message is required" }, { status: 400 })
        }
        ...
        if (!setting) {
            return NextResponse.json({ message: "chatbot is not configured yet :(" }, { status: 400 })
        }
...
- Clear and concise
- Helpful and polite
- Limited to 2–5 sentences unless necessary
...
`;
        ...
        const response = NextResponse.json({ message: "chat error" }, { status: 500 })
        response.headers.set("Access-Control-Allow-Origin", "*");
        response.headers.set("Access-Control-Allow-Methods", "POST, OPTIONS");
        response.headers.set("Access-Control-Allow-Headers", "Content-Type");
